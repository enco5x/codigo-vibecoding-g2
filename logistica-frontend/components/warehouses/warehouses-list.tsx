...
interface Props {
  onEdit: (warehouse: WarehouseList) => void
  onDelete: (warehouse: WarehouseList) => void
  onCreate: () => void
}

export function WarehousesList({ onEdit, onDelete, onCreate }: Props) {
  const [page, setPage] = useState(1)
  const [sorting, setSorting] = useState<SortingState>([])
  const [search, setSearch] = useState("")
  const [activeFilter, setActiveFilter] = useState<"all" | "active" | "inactive">("all")
  const { data, isLoading, isError, refetch } = useWarehouses(page)
  const user = useAuthStore((s) => s.user)
  const groups = user?.groups ?? []
  const permissions = user?.permissions ?? []
  const isSuperuser = user?.is_superuser ?? false
  const canWrite = canAccess("warehouses", "write", groups, permissions, isSuperuser)
      ...
      helper.display({
        ...
        cell: ({ row }) => (
          <div className="flex gap-1 justify-end">
            {canWrite && (
              <Button variant="ghost" size="icon-xs" onClick={() => onEdit(row.original)} className="text-white/40 hover:text-white/80" data-testid="warehouses-edit">
                <Pencil className="h-4 w-4" />
              </Button>
            )}
            {canWrite && (
              <Button variant="ghost" size="icon-xs" onClick={() => onDelete(row.original)} className="text-white/40 hover:text-red-400" data-testid="warehouses-delete">
  ...
  const filteredData = useMemo(() => {
    ...
    if (activeFilter === "active") items = items.filter((w) => w.is_active)
    if (activeFilter === "inactive") items = items.filter((w) => !w.is_active)
    return items
  ...
  if (isError) {
    return (
      <div className="flex flex-col items-center gap-4 py-20">
        <p className="text-white/50">Error al cargar bodegas</p>
