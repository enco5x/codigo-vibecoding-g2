...
import { useGroups } from "@/lib/hooks/use-users"
...
import { useDeleteGroup } from "@/lib/hooks/use-users"
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export function GroupsTable() {
  ...
  const columns = useMemo(
    () => [
      ...
      helper.accessor("permissions", {
        header: "Permisos",
        cell: (info) => (
          <span className="text-white/50">{info.getValue().length} permiso{info.getValue().length !== 1 ? "s" : ""}</span>
        ),
      }),
      helper.display({
        id: "actions",
        ...
        cell: ({ row }) => (
          <div className="flex gap-1 justify-end">
            <Button
              variant="ghost"
              size="icon-xs"
              onClick={() => router.push(`/users/groups/${row.original.id}`)}
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((g) => g.name.toLowerCase().includes(search.toLowerCase()))
  }, [data, search])
  ...
  if (isError) {
    return (
      <div className="flex flex-col items-center gap-4 py-20">
        <p className="text-white/50">Error al cargar roles</p>
    ...
    <>
      <div className="space-y-4">
        <DataTableToolbar
          ...
          action={
            <Button
              onClick={() => router.push("/users/groups/nuevo")}
      ...
      <Dialog open={!!deleting} onOpenChange={() => setDeleting(null)}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              ¿Eliminar rol <strong>{deleting?.name}</strong>? Esta acción no se puede deshacer.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
